		...
		<div class="row">
		   ...
		   <!-- /.box -->
                ...
                <th>Actual Earning</th>
							...
							<tbody id = "ts">
							<?php $i = 1; foreach($finalList as $data){ ?>
								<tr>
									<td><?= $i;?></td>
									<td>
                    <?php
                      $date1=date_create($data['created']);
                      echo date_format($date1,"Y-m-d");
                    ?>
                  </td>
									<td><?php  echo $data['recivedCoin'];?></td>
                  <td>
                    <?php
                      $hours =  floor($data['liveDuration']/60); // No. of mins/60 to get the hours and round down
                      $mins =   $data['liveDuration'] % 60; // No. of mins/60 - remainder (modulus) is the minutes
                      echo $hours . " h " . $mins . " m ";
                    ?>
                  </td>
                  <td><?php  echo $data['validDays'];?></td>
