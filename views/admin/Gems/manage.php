		   ...
		   <!-- /.box -->
				<div class="box">
					<div class="box-header">
						<h3 class="box-title"><a href="<?= site_url();?>/Gems/add" style="font-size: 14px;" class="btn btn-block btn-success btn-xs">Add Gems</a></h3>
					</div>
											...
											<li class="dropdown">
												...
												<ul class="dropdown-menu dropdown-menu-right">
													<li role="presentation">
														<a role="menuitem" tabindex="-1" href="<?= site_url()?>/Gems/edit/<?= $data['id'];?>">Edit</a>
													</li>
													<li>
														<a role="menuitem" tabindex="-1" onclick="return confirm(&quot;Once you've deleted 	this account, the gems (<?php  echo $data['title'];?>) will no longer be able to log in to the site or apps. This action cannot be undone.&quot;);" href="<?= site_url()?>/Gems/delete/<?= $data['id'];?>">Delete
											            </a>
													</li>
													<li role="presentation" class="divider"></li>	
											        <li role="presentation">
														<a role="menuitem" onclick="return confirm(<?php  echo $data['username'];?> - Do you want <?php if($data['status'] == 'Approved'){ echo "Decline"; } else{ echo "Approve";}?> this gems&quot;);" tabindex="-1" href="<?= site_url()?>/Gems/status/<?= $data['id'];?>">
    	...
    	$.ajax({
            ...
            if(data ==''){
                ...
                for(var i = 0; i < data.length; i++) {
                	comment = data[i];
           			html += '<tr><td>' + (i+1) + '</td><td>' + comment.title + '</td><td>' + img + '</td><td>' + comment.count + 
                    '</td><td>' + comment.price + '</td><td>' + "0" +
