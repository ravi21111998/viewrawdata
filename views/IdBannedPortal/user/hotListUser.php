	...
	<section class="content">
		<div class="row">
          ...
          <div class="box-header">
            <!-- <div <?php  if($admin['role'] == 'subAdmin'){?> style="display:none" <?php } ?> ><h3 class="box-title"><a href="<?= site_url();?>/User/addHotList" style="font-size: 14px;" class="btn btn-block btn-success btn-xs">Add Hotlist User</a></h3></div> -->
                ...
                <th>Phone</th>
							...
							$i = 1; foreach($details as $data){ ?>
								<tr>
									<td><?= $i;?></td>
                  <td>
                    <?php if(!empty($data['imageDp'])){?>
                      <img src="<?php echo $data['imageDp']?>" style="width:80px">
                    <?php } else{?>
                  ...
                  <td><?php  echo $data['phone'];?></td>
									<!-- <td><button type="button" onclick="remove(this.id)" id="<?php echo $data['id']?>" class="btn btn-info">Remove</button></td> -->
    ...
    $.ajax({
      ...
      dataType: 'json',                //data format
          success: function(data){
            swal("Good job!", "User removed from hotlist", "success");
